...
async function getData(segment) {
  try {
    ...
    console.log('№1.Отримані дані:', data);
  } catch (error) {
    console.error("Сталася помилка:", error.message);

    return `Сталася помилка при виконанні запиту. ${error.message}`;
  };
};
...
async function postData(segment, data) {
  try {
    ...
    if (!response.ok) {
      ...
      console.log('№2.Отримані дані:', responseData);
      return responseData;
    };
  } catch (error) {
    console.error("Сталася помилка:", error.message);
...
async function putData(id, data) {
  try {
    ...
    if (!response.ok) {
      ...
      const responseData = await response.json();
      console.log('№3.Отримані дані:', responseData);
      return responseData;
    };
  } catch (error) {
    console.error("Сталася помилка:", error.message);
...
async function patchData(id, data) {
  try {
    ...
    if (!response.ok) {
      ...
      const responseData = await response.json();
      console.log('№4. Отримані дані:', responseData);
      return responseData;
    };
  } catch (error) {
    console.error("Сталася помилка:", error.message);
...
async function deleteData(id) {
  try {
    ...
    if (!response.ok) {
      throw new Error(`Не вдалося видалити пост з id ${id}. Статус: ${response.status}`); 
    } else {
      console.log(`№5. Пост з id [${id}] успішно видалено.`);
      return true; 
    };
  } catch (error) {
    console.error(`Помилка під час видалення: ${error.message}`);
    return `Помилка під час видалення: ${error.message}`; 
  };
};
